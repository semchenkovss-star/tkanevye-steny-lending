...
import { ACOUSTIC_POSTS } from '@/data/blog';
...
const AcBlog = () => (
  <Section
    ...
    eyebrow="Блог"
    title={<>Про шум и эхо</>}
    lead="Статьи о звукоизоляции и акустике комнаты: откуда берётся эхо, как гасится ударный шум от соседей, что делать с гулкой бетонной коробкой и чем ткань отличается от войлока."
    tone="surface"
  >
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      {ACOUSTIC_POSTS.map((p) => (
        <Link
          key={p.slug}
          to={`/blog/${p.slug}`}
        ...
        >
          <div className="aspect-[4/3] overflow-hidden">
            <img
              src={p.img}
              alt={`${p.title} — статья о шумоизоляции и акустике комнаты`}
              loading="lazy"
