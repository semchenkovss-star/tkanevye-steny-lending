...
import { CEIL_FAQ } from '@/lib/ceilings';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const PROMISES = [
  ...
  'Образцы полотен привозим с собой, смотрите при своём свете',
    ...
    <div className="grid gap-10 lg:grid-cols-12">
      <div className="lg:col-span-10 lg:col-start-2">
        <Accordion type="single" collapsible className="border-t border-border">
          {CEIL_FAQ.map((item, i) => (
            <AccordionItem key={item.q} value={`ceil-${i}`} className="border-border">
        ...
        <div className="grid gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-6 lg:col-start-7">
            <div className="border border-border bg-card p-6 sm:p-8 lg:p-10">
              <LeadForm source="Потолки — форма внизу" />
            </div>
          </div>
        </div>
      </div>
