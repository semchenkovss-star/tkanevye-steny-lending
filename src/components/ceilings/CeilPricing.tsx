...
const CeilPricing = () => (
  <Section
    ...
    lead="В стоимость натяжного потолка под ключ входит полотно, багет, работа бригады и уборка. Итог фиксируется в смете после замера и больше не меняется."
    tone="surface"
  >
    <div className="grid gap-px border border-border bg-border lg:grid-cols-3">
      {CEIL_PLANS.map((p) => (
        ...
        >
          ...
          <p className={`mt-3 text-sm ${p.accent ? 'text-background/60' : 'text-muted-foreground'}`}>
            {p.for}
          </p>

          <div className="mt-8 flex items-baseline gap-2">
            <span className="font-display text-5xl leading-none text-primary">{p.priceLabel}</span>
            <span className={`text-sm ${p.accent ? 'text-background/60' : 'text-muted-foreground'}`}>
              ₽ / м²
            </span>
          </div>
          ...
          <button
            type="button"
            onClick={() => openLead(`Потолки — тариф «${p.name}»`)}
    ...
    <div className="mt-10 flex flex-col items-start justify-between gap-6 border border-border bg-card p-8 sm:flex-row sm:items-center lg:p-10">
      <div>
        <h3 className="font-display text-2xl uppercase leading-none tracking-wide sm:text-3xl">
          Хотите точную цифру по своей комнате?
        </h3>
        <p className="mt-3 max-w-[34em] text-sm leading-[1.55] text-muted-foreground">
          Введите длину и ширину комнаты в калькуляторе — стоимость посчитается сразу.
        </p>
      </div>
      <a
        href="#ceil-calc"
